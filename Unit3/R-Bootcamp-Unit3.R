...
# Unlike simple dates, date-times provide information on both the date and the time, including additional formatting options:

format(Sys.time(), "%d %b %Y") # just the date
format(Sys.time(), "%d %b %Y %T %r %z %Z")
...
# In addition, note that time zone matters when working with both dates and date-times. By default, R will assume dates and date-times are being specified in your system's current time zone (e.g., EDT, or Eastern Daylight Time):

(time3 <- as.POSIXct("2019-09-01 12:00:00")) # default time zone for your system
format(time3, "%F %T %z %Z") # showing time zone and offset from coordinated universal time (UTC)

# You can specify an alternate time zone for a date using the tz argument:

(time4 <- as.POSIXct("2019-09-01 12:30:45", tz = "UTC"))

# Using the attributes function, it is possible to inspect the time zone of a date-time object:

attributes(time4)

# Moreover, it is also possible to change the time zone using this function:

attributes(time4)$tzone <- "EST5EDT" # EST5EDT is the formal representation for eastern daylight time (i.e., EDT) with a variable offset based on daylight savings
time4
...
# Although outside of the scope of this bootcamp, various aspects of time including time zones, daylight savings time, and leap years can add significant complexity to working with dates. For cases such as these, take a look at the lubridate and anytime packages, which can significantly simplify work involving with dates/times and avoid simple mistakes. See the following unit for more on working with packages.